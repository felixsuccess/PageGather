...
interface BookGroupDao {
    ...
    /** 获取所有分组（按排序顺序） */
    @Query("SELECT * FROM book_group WHERE is_deleted = 0 ORDER BY group_order ASC")
    ...
    /** 根据名称搜索分组 */
    @Query("SELECT * FROM book_group WHERE name LIKE '%' || :name || '%' AND is_deleted = 0 ORDER BY group_order ASC")
