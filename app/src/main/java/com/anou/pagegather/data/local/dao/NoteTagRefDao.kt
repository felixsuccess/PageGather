...
interface NoteTagRefDao {
    ...
    /** 获取笔记的所有标签关联 */
    @Query("SELECT * FROM note_tag_ref WHERE note_id = :noteId AND is_deleted = 0")
    fun getTagRefsByNoteId(noteId: Long): Flow<List<NoteTagRefEntity>>
    
    /** 获取标签的所有笔记关联 */
    @Query("SELECT * FROM note_tag_ref WHERE tag_id = :tagId AND is_deleted = 0")
    fun getNoteRefsByTagId(tagId: Long): Flow<List<NoteTagRefEntity>>
    
    /** 检查笔记是否有指定标签 */
    @Query("SELECT COUNT(*) FROM note_tag_ref WHERE note_id = :noteId AND tag_id = :tagId AND is_deleted = 0")
    suspend fun isNoteHasTag(noteId: Long, tagId: Long): Int
    
    /** 获取标签下的笔记数量 */
    @Query("SELECT COUNT(*) FROM note_tag_ref WHERE tag_id = :tagId AND is_deleted = 0")
    ...
    /** 从笔记移除标签（软删除） */
    @Query("UPDATE note_tag_ref SET is_deleted = 1, updated_date = :updateTime WHERE note_id = :noteId AND tag_id = :tagId")
    suspend fun removeTagFromNote(noteId: Long, tagId: Long, updateTime: Long = System.currentTimeMillis())
    
    /** 移除笔记的所有标签（软删除） */
    @Query("UPDATE note_tag_ref SET is_deleted = 1, updated_date = :updateTime WHERE note_id = :noteId")
    suspend fun removeAllTagsFromNote(noteId: Long, updateTime: Long = System.currentTimeMillis())
    
    /** 移除标签的所有笔记关联（软删除） */
    @Query("UPDATE note_tag_ref SET is_deleted = 1, updated_date = :updateTime WHERE tag_id = :tagId")
